//
// Created by seb on 19/07/2020.
//

#include <include/Helper.hpp>
#include <src/SnakeWrapper.hpp>
#include <include/NEATERHelper.hpp>
#include "NEATERSpecie.hpp"

NEATERSpecie::NEATERSpecie(ioTable input, ioTable output, size_t brainCycle) : deadline(2), input(input), output(output), brainCycle(brainCycle) {
	population.reserve(SPECIE_SIZE);
	for (int i = 0; i < SPECIE_SIZE; ++i)
		population.emplace_back(std::make_shared<NEATERNeuralNetwork>(input, output, brainCycle));
	std::random_device rd;
	std::mt19937 gen(rd());
	std::uniform_int_distribution<> dis(0, 255);
	color = sf::Color(dis(gen), dis(gen), dis(gen));
}

NEATERSpecie::NEATERSpecie(std::shared_ptr<NEATERNeuralNetwork> ann) {
	population.reserve(SPECIE_SIZE);
	for (int i = 0; i < SPECIE_SIZE; ++i) {
		population.emplace_back(std::make_shared<NEATERNeuralNetwork>(input, output, brainCycle));
		population[i]->crossover(ann);
	}
	std::random_device rd;
	std::mt19937 gen(rd());
	std::uniform_int_distribution<> dis(0, 255);
	color = sf::Color(dis(gen), dis(gen), dis(gen));
}

void NEATERSpecie::update(ulong seed) {

//	std::vector<std::thread> workers;
	for (int i = 0; i < population.size(); ++i) {
		/*if (i % NBTHREAD == 0) {
			if (!workers.empty()) {
				for (auto &elem : workers)
					elem.join();
				workers.clear();
			}
			workers.reserve(NBTHREAD);
		}
		auto process = population[i];
		workers.emplace_back(*/
		auto process = population[i];
		[process, seed]() {
		    SnakeWrapper ale(GRID_SIZE, GRID_SIZE, seed, false);
		    process->fitness = -1;
		    while (process->fitness == -1) {
			    auto data = ale.getData();
			    process->setInput(data);
			    process->update();
			    process->fitness = ale.sendAction(process->getOutput());
		    }
		}();//);
	}
//	for (auto &worker : workers)
//		worker.join();

	sort();

	averageFitness = 0.0f;
	for (auto const &elem : population)
		averageFitness += elem->fitness;
	averageFitness = averageFitness / population.size();

	for (auto &elem: population) {
		if (elem->mutationIterator != -1) {
			auto it = std::next(globalMutations.begin(), elem->mutationIterator); ///wtf pk mutationIterator est à -1000000
			if (it->deltaFitness != -1)
				it->deltaFitness = elem->fitness - it->deltaFitness;
		}
	}
}

void NEATERSpecie::crossover() {
	std::random_device rd;
	std::mt19937 gen(rd());
	std::uniform_int_distribution<> dis(0, population.size() * (ELITE / 100.0));
	std::uniform_real_distribution<float> prcent(0.f, 1.0f);

	for (int i = population.size() * (ELITE / 100.0) + 1; i < population.size(); ++i) {
		population[i]->crossover(population[dis(gen)]);
	}
}

void NEATERSpecie::mutate() {

	for (int i = population.size() * (ELITE / 100.0) + 1; i < population.size(); ++i) {
		population[i]->mutation();
	}
}

void NEATERSpecie::sort() {
	std::sort(population.begin(), population.end(),
		  [](std::shared_ptr<NEATERNeuralNetwork> const &a, std::shared_ptr<NEATERNeuralNetwork> const &b) {
		      return a->fitness > b->fitness;
		  });
}